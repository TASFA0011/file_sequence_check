...
class UploadFacturationAdress extends StatefulWidget {
  final bool? isFirst;
  const UploadFacturationAdress({super.key, this.isFirst});

  @override
  State<UploadFacturationAdress> createState() => _UploadFacturationAdressState();
}

class _UploadFacturationAdressState extends State<UploadFacturationAdress> {
  
  late final FormValidatorBloc _bloc;


  FormValidatorBloc registerBloc({File? file}) {
  return FormValidatorBloc(
      data: FormValidatorData(
          fields: HashMap.from({
    'file': ValidatorField<Zfile?>(
      file != null ? Zfile.fromFile(file): null,
      requiredMessage: "Veuillez charger un fichier excel contenant les adresses"
    )
  ...
  void _onSubmit() {
    log(name: 'data', _bloc.validate().toString());
    if (_bloc.data.isLoading || !_bloc.validate()) {
      return;
    }
    _bloc.setLoadingStatus(true);

    Api.uploadAdress(_bloc.getFormValues()).then((value) async {
      ...
      NavigationService.nestedNavigatorKey.currentState?.pushNamedAndRemoveUntil(Routes.FACTURATIONS, (_)=>false);
    }, onError: (err) {
      _bloc.setLoadingStatus(false);
        ...
        child: Column(
          children: [
            ...
            if(widget.isFirst == true)
              const Text(
                'Veuillez charger d\'abord les adresses de la facturation',
            ...
            SizedBox(
              width: width / 3,
              child: SingleFilePicker(onChanged: (file) => _onChange("file", file))
